...
def test_defaults_when_no_files_no_env(monkeypatch):
    for var in ("MAGI_OLLAMA_HOST", "OLLAMA_HOST", "MAGI_OLLAMA_API_KEY",
                "OLLAMA_API_KEY", "MAGI_OLLAMA_MODEL_MELCHIOR",
                "MAGI_OLLAMA_MODEL_BALTHASAR", "MAGI_OLLAMA_MODEL_CASPAR"):
...
def test_env_overrides_files(tmp_path):
    ...
    cfg = resolve_config(global_path="/nope.toml", repo_path=r,
                         env={"MAGI_OLLAMA_MODEL_CASPAR": "ec"})
    assert cfg.models["caspar"] == "ec"


def test_ollama_host_is_fallback_below_files(tmp_path):
    cfg = resolve_config(global_path="/nope.toml", repo_path="/nope.toml",
                         env={"OLLAMA_HOST": "1.2.3.4:11434"})
...
def test_base_url_normalization(raw, expected):
    cfg = resolve_config(global_path="/n.toml", repo_path="/n.toml",
                         env={"MAGI_OLLAMA_HOST": raw})
    assert cfg.base_url == expected


def test_api_key_precedence(tmp_path):
    g = _write(tmp_path / "g.toml", 'api_key="gk"\n')
    cfg = resolve_config(global_path=g, repo_path="/n.toml",
                         env={"OLLAMA_API_KEY": "ok"})
    assert cfg.api_key == "gk"  # file beats generic OLLAMA_API_KEY
    cfg2 = resolve_config(global_path=g, repo_path="/n.toml",
                          env={"MAGI_OLLAMA_API_KEY": "mk"})
