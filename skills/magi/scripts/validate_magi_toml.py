...
def main() -> int:
    ...
    # Grieta 2: echo the RESOLVED value -- defaults to False (a family contradiction
    # only warns), so a config that never sets this key inherits the warn-only path.
    print(f"  strict_lineage = {config.strict_lineage}")
